...
| Triggers & Stored Procedures | Auto seat allocation (AllocateSeat), prevent double booking, update payment status, auto-cancel bookings, check available seats |
...
| Query | Description |
|-------|-------------|
| `SELECT f.flight_number, da.airport_code AS departure, aa.airport_code AS arrival, f.departure_time, f.arrival_time, a.model AS aircraft_model, f.status FROM Flights f JOIN Airports da ON f.departure_airport = da.airport_id JOIN Airports aa ON f.arrival_airport = aa.airport_id JOIN Aircraft a ON f.aircraft_id = a.aircraft_id;` | View all flights |
| `SELECT p.full_name, f.flight_number, s.seat_number, b.booking_status, b.booking_date FROM Bookings b JOIN Passengers p ON b.passenger_id=p.passenger_id JOIN Flights f ON b.flight_id=f.flight_id JOIN Seats s ON b.seat_id=s.seat_id;` | View passenger bookings |
| `SELECT f.flight_number, SUM(pay.amount) AS total_revenue FROM Flights f JOIN Bookings b ON f.flight_id=b.flight_id JOIN Payments pay ON b.booking_id=pay.booking_id WHERE pay.payment_status='Success' GROUP BY f.flight_number;` | View total revenue per flight |
| `CALL CheckAvailableSeats(1);` | Check available seats for a flight |
| `CALL AllocateSeat(1, 1);` | Allocate a seat for a passenger |
